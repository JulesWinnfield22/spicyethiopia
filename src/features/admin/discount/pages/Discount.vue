...
const pagination = usePagination({
  ...
  cb: (data: any) => getAllDiscounts({status: activeFilter.value !== "ALL" ? activeFilter.value : undefined, ...data}),
  watch: [activeFilter]
});
...
<template>
  <AdminDefaultPageWrapper>
    <template #actions>
      <Button @click="openModal('AddDiscount')" type="secondary"> Add Discount </Button>
    </template>
          ...
          <DropdownParent
            :items="[
              ...
              },
              {
                ...
                action: () =>
                  console.log('Delete Discount clicked for', row.id),
              },
            ]"
          />
        </Dropdown>
