...
const handleLogin = ({values}: any) => {
  ...
  req.send(
    ...
    res => {
      if(res.success) {
        localStorage.setItem('spicy_user', JSON.stringify(res.data))
  ...
  <div class="grid grid-cols-3 h-full box-border">
    ...
    <div class="flex flex-col p-8">
      <div class="w-full max-w-md space-y-8">
        ...
        <!-- Welcome Text -->
        <div>
          <h2 class="font-dm-serif text-xl  mb-5">
        ...
        <Form v-slot="{ submit }" id="login-form" class="grid gap-4">
          ...
          <Button :pending="req.pending.value" @click.prevent="submit(handleLogin)" size="md" type="secondary"> Login </Button>
        </Form>
