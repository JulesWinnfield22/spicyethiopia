...
<template>
  <div @mouseout="out" @mouseover="stay" ref="t"  :class="[$style[toast?.type || 'error']]"
    class="relative rounded-md max-w-[50%] flex items-center p-4 gap-3 text-base shadow-md min-w-72 justify-between">
    <p class="text-sm">{{ toast?.message }}</p>
    <button @click="removeT" class="min-w-4 min-h-4 flex-shrink-0 bg-dark/20 shadow-md rounded-full text-base-clr2 right-0 top-0 w-8 h-8 flex items-center justify-center self-start">
...
.error {
   ...
   border-left: 1rem solid var(--color-red-500);
}
