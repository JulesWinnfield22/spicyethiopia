...
<template>
  <InputParent :attributes="{...(attributes || {}), type: 'text'}" v-model="value" v-slot="{ setRef, error, value, attributes, changeValue }">
    <InputLayout
    ...
    >
      <div class="flex items-center h-full flex-1">
        ...
        <select
          ...
          :ref="setRef"
          class="appearance-none !pl-3 !pr-8 flex-1 text-text-clr h-full text-sm"
        >
          <option
            selected
            value=""
        ...
        <div class="absolute pointer-events-none top-0 right-0 h-full">
          <slot name="right">
            <div class="h-full ml-auto w-8 flex items-center justify-center">
              <i v-if="!pending" v-html="icons.down" />
