...
const initialValue = ref(
  props.modelValue ? cloneValue(props.modelValue) : props.value,
);
...
function updateSkip() {
  if (props.skip) {
    inputEl.value?.setAttribute?.("data-skip", "true");
  } else {
    inputEl.value?.removeAttibutes?.("data-skip");
  }
}

onMounted(updateSkip);
watch(
  () => props.skip,
  () => {
    updateSkip();
...
function updateEl(setError = true) {
  if (initialAttrs.value?.type == "file" || props.saveOnEl) {
...
if (isRef(validateAll) || isReactive(validateAll)) {
  watch(validateAll, () => {
...
if (isRef(reset) || isReactive(reset)) {
  watch(reset, () => {
    if (reset.value) {
